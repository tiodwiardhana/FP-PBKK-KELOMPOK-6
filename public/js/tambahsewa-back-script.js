...
//untuk tambah kartu
    function tambahKartu(tanggal,kode_studio, lokasi, kode_jadwal ,jam, harga) {

      var div = document.createElement("DIV");
      var n_id = "k"+kode_studio;
      div.setAttribute('id',n_id);
      div.setAttribute('class','kartubaru');
      div.innerHTML =
      "<div class=\"card\">"+
      "<style>"+ "#"+kode_studio+"{background: #95a5a6; color:white; pointer-events: none; cursor: not-allowed;}" +"</style>"+
          "<div class=\"content\">"+
                  "<input type=\"text\" hidden name=\"tanggal_jadwal[]\" value=\"" + tanggal + "\" >"+
                  "<input type=\"text\" hidden name=\"kode_jadwal[]\" value=\"" + kode_jadwal + "\" >"+
                  "<input type=\"hidden\" name=\"kode_studio[]\" value=\"" + kode_studio + "\">"+
                  "<div class=\"row\">"+
                      "<div class=\"col-md-2\">"+
            ...
            $.ajax({
               ...
               success: function(response){
                  ...
                  for(i = 0 ; i < response['data'].length ; i++){
                    ...
                    if(response['data'][i].kode_studio == id_btn){
                      alert(response['data'][i].kode_studio);
                      tambahKartu(tanggal,response['data'][i].kode_studio,response['data'][i].lokasi,response['data'][i].kode_jadwal,response['data'][i].jadwal.jam, response['data'][i].jadwal.harga);
                    }
                  }
               }
            });
